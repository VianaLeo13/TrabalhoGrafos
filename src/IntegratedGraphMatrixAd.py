...
class IntegratedGraphMatrixAd:
    ...
    def __init__(self):
        """
        Construtor do grafo integrado usando matriz de adjacência.
    ...
    def _extrair_usuarios(self):
        ...
        print(f"Total de usuários únicos encontrados (Grafo Integrado - Matriz): {len(usuarios_set)}")
    ...
    def _construir_grafo(self):
        ...
        """
        print("Construindo grafo integrado com pesos (Matriz de Adjacência)...")
        ...
        print(f"✅ Grafo Integrado (Matriz) construído com {self.grafo.getVertexCount()} vértices e {self.grafo.getEdgeCount()} arestas")
    ...
    def imprimir_estatisticas(self):
        ...
        """
        if not self.grafo:
            print("Grafo Integrado (Matriz) não foi construído!")
            return
        
        print("=" * 70)
        print("ESTATÍSTICAS DO GRAFO INTEGRADO - TODAS AS INTERAÇÕES (MATRIZ)")
        print("=" * 70)
    ...
    def exportToGEPHI(self, path: str) -> None:
        ...
        try:
            with open(path, 'w', encoding='utf-8') as f:
                ...
                f.write('    <creator>TrabalhoGrafos - IntegratedGraphMatrixAd</creator>\n')
                f.write('    <description>Grafo integrado com todas as interações GitHub (ponderado - Matriz)</description>\n')
                f.write('  </meta>\n')
            ...
            print(f"Grafo Integrado (Matriz) exportado com sucesso para: {path}")
            ...
            print(f"Erro ao exportar grafo integrado (Matriz): {e}")
            raise


def main():
    """
    Função principal para testar o grafo integrado (Matriz).
    """
    print("Criando grafo integrado com todas as interações (Matriz de Adjacência)...")
    
    try:
        # Cria o grafo integrado usando matriz
        integrated_graph = IntegratedGraphMatrixAd()
        ...
        # Exporta para GEPHI
        print("\n" + "=" * 60)
        print("Exportando grafo integrado (Matriz) para visualização no GEPHI...")
        integrated_graph.exportToGEPHI("grafo_integrado_ponderado_matrix.gexf")
        
    except Exception as e:
        print(f"Erro ao criar grafo integrado (Matriz): {e}")
        import traceback
        traceback.print_exc()
