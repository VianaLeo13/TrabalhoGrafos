...
class PullRequestReviewGraph:
    ...
    def _extrair_usuarios(self):
        ...
        print(f"Total de usuários únicos encontrados (Grafo 3): {len(usuarios_set)}")
    ...
    def _construir_grafo(self):
        ...
        """
        print("Construindo grafo de revisões/merges de pull requests (Grafo 3)...")
    ...
    def imprimir_estatisticas(self):
        ...
        """
        if not self.grafo:
            print("Grafo 3 não foi construído!")
            return
        
        print("=" * 50)
        print("ESTATÍSTICAS DO GRAFO 3 - REVISÕES/MERGES DE PRS")
        print("=" * 50)
    ...
    def exportToGEPHI(self, path: str) -> None:
        ...
        try:
            with open(path, 'w', encoding='utf-8') as f:
                ...
                f.write('    <creator>TrabalhoGrafos - PullRequestReviewGraph</creator>\n')
                f.write('    <description>Grafo de revisões/merges de PRs GitHub para visualização no GEPHI</description>\n')
                f.write('  </meta>\n')
...
def main():
    """
    Função principal para testar o grafo de revisões/merges (Grafo 3).
    """
    print("Criando grafo de revisões/merges de pull requests (Grafo 3)...")
    
    try:
        # Cria o grafo de revisões/merges
        pr_review_graph = PullRequestReviewGraph()
        ...
        # Exporta para GEPHI
        print("\n" + "=" * 50)
        print("Exportando grafo 3 para visualização no GEPHI...")
        pr_review_graph.exportToGEPHI("grafo_reviews_merges_prs.gexf")
        
    except Exception as e:
        print(f"Erro ao criar grafo de revisões/merges (Grafo 3): {e}")
        import traceback
        traceback.print_exc()
